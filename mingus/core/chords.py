...
recognise a plethora of chords. The following overview groups some of the \
functions you are most likely to use together.
    ...
    for n in rest_of_string:
        if n == '/':
            ...
            return from_shorthand(shorthand_string[:len(name) + s],
                                  from_shorthand(shorthand_string[len(name) + s
                                   + 1:]))
        s += 1
...
def determine(
    ...
    """Names a chord. Can determine almost every chord, from a simple triad to a \
...
def determine_triad(
    ...
    """Names the triad. Returns answers in a list. The third argument should not be \
given. If shorthand is True the answers will be in abbreviated form.

Can determine major, minor, diminished and suspended triads. Also knows \
about invertions.
...
def determine_seventh(
    ...
    """Determines the type of seventh chord. Returns the results in a lists, \
ordered on inversions. Expects `seventh` to be a list of 4 notes. If \
`shorthand` is set to True, results will be returned in chord shorthand \
...
def determine_polychords(chord, shorthand=False):
    """Determines the polychords in chord. Can handle anything from polychords \
based on two triads to 6 note extended chords."""
