...
def to_chords(progression, key='C'):
    """Converts a list of chord functions (eg `['I', 'V7']`) or a string (eg. 'I7') \
to a list of chords. Any number of accidentals can be used as prefix to \
augment or diminish; for example: bIV or #I. All the chord abbreviations in \
the chord module can be used as suffixes; for example: Im7, IVdim7, etc. You \
can combine prefixes and suffixes to manage complex progressions: #vii7, \
#iidim7, iii7, etc. Using 7 as suffix is ambiguous, since it is classicly \
used to denote the seventh chord when talking about progressions instead of \
_just_ the dominanth seventh chord. We have taken the classic route; I7 will \
get you a major seventh chord. If you specifically want a dominanth seventh, \
use Idom7."""
...
def determine(chord, key, shorthand=False):
    """Determines the harmonic function of chord in key. This function can also \
deal with lists of chords.
{{{
>>> determine([\"C\", \"E\", \"G\"], \"C\")
['tonic']
>>> determine([\"G\", \"B\", \"D\"], \"C\")
...
for `progression[substitute_index]`. If `ignore_suffix` is set to True the \
suffix of the chord being substituted will be ignored. Otherwise only \
progressions without a suffix, or with suffix '7' will be substituted. The \
following table is used to convert progressions:
|| I || III ||
|| I || VI ||
|| IV || II ||
|| IV || VI ||
|| V || VII ||"""
    ...
    simple_substitutions = [('I', 'III'), ('I', 'VI'), ('IV', 'II'), ('IV', 'VI'
                            ), ('V', 'VII')]
    res = []
...
def substitute_minor_for_major(progression, substitute_index,
                               ignore_suffix=False):
    """Substitutes minor chords for its major equivalent. Recognizes 'm' and 'm7' \
suffixes and ['II', 'III', 'VI'] if there is no suffix.
{{{
...
def substitute_major_for_minor(progression, substitute_index,
                               ignore_suffix=False):
    """Substitutes major chords for their minor equivalent. Recognizes 'M' and 'M7' \
suffixes and ['I', 'IV', 'V'] if there is no suffix.
{{{
...
[\"IIIm7\"]
    ...
    if suff == 'M' or suff == 'M7' or suff == '' and roman in ['I', 'IV', 'V']\
         or ignore_suffix:
        n = skip(roman, 5)
...
def substitute_diminished_for_diminished(progression, substitute_index,
        ignore_suffix=False):
    """Substitutes a diminished chord for another diminished chord. Recognizes the \
'dim' and 'dim7' suffix and \"VI\" if there is no suffix.
{{{
...
def substitute(progression, substitute_index, depth=0):
    """Gives a list of possible substitutions for `progression[substitute_index]`. \
If depth > 0 the substitutions of each result will be recursively added as \
well.
{{{
...
def interval_diff(progression1, progression2, interval):
    """Returns the number of half steps progression2 needs to be diminished or \
