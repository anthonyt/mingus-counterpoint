...
================================================================================
    ...
    The fft module can find the frequencies in raw audio data by using fast \
Fourier
    transformations (supplied by numpy) and can also convert the found \
frequencies
    to Note objects.
...
def data_from_file(file):
    """Returns (first channel data, sample frequency, sample width) from a .wav \
file"""
...
def find_Note(data, freq, bits):
    """Gets the frequencies, feeds them to find_notes and the returns the Note with \
...
def analyze_chunks(
    ...
    """Cuts the one channel data in chunks and analyzes them separately. Making the \
chunksize a power of two works fastest."""
...
def find_melody(file='440_480_clean.wav', chunksize=512):
    """Experimental function: cuts the sample into chunks and analyzes each chunk. \
If two consequent chunks turn out to return the same Note they are grouped \
together. This function returns a list [(Note, chunks)] where chunks is the \
number of chunks where that note is the most dominant."""
