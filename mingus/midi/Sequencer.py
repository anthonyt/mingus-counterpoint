...
================================================================================
    ...
    can use the Sequencer object either by creating a subclass and implementing
    some of the events (`init`, `play_event`, `stop_event`, `cc_event`, \
`instr_event`)
    or by attaching observer objects via `attach` and catching the
    messages in the notify(msg_type, param_dict) function of your object.
    See SequencerObserver for a pre made, easy to extend base class that
    can be attached to the Sequencer.
...
of the events (`init`, `play_event`, `stop_event`, `cc_event`, \
...
attached to the Sequencer."""
    ...
    def attach(self, listener):
        """Attach an object that should be notified of events. The object should \
have a notify(msg_type, param_dict) function."""
    ...
    def set_instrument(
        ...
        self.notify_listeners(self.MSG_INSTR, {'channel': int(channel), 'instr'
                              : int(instr), 'bank': int(bank)})
    ...
    def control_change(
        ...
        """Sends a control change message. See the MIDI specification for more \
information."""
        ...
        self.notify_listeners(self.MSG_CC, {'channel': int(channel), 'control'
                              : int(control), 'value': int(value)})
        return True
    ...
    def play_Note(
        ...
        """Plays a Note object on a channel with a velocity[0-127]. You can either \
specify the velocity and channel here as arguments or you can set the \
Note.velocity and Note.channel attributes, which will take presedence \
over the function arguments."""
    ...
    def stop_Note(self, note, channel=1):
        """Stops a note on a channel. If Note.channel is set, it will take \
presedence over the channel argument given here."""
    ...
    def play_NoteContainer(
        ...
        self.notify_listeners(self.MSG_PLAY_NC, {'notes': nc, 'channel'
    ...
    def play_Bar(
        ...
        """Plays a Bar object. The tempo can be changed by setting the bpm \
attribute on a NoteContainer. Returns a dictionary with the bpm lemma \
set on success, an empty dict on some kind of failure."""
...
channels should also be provided. The tempo can be changed by providing \
one or more of the NoteContainers with a bpm argument."""

        self.notify_listeners(self.MSG_PLAY_BARS, {'bars': bars, 'channels'
                              : channels, 'bpm': bpm})
        ...
        while tick < bars[0].length:
            ...
            # If somehow, playing_new doesn't contain any notes (something that
            # shouldn't happen when the bar was filled properly), we make sure
            # that at least the notes that are still playing get handled
            # correctly.
    ...
    def play_Tracks(
        ...
        """Plays a list of Tracks. If an instance of MidiInstrument is used then \
the instrument will be set automatically."""
