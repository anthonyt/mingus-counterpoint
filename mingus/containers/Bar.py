...
in [refMingusContainersTrack Tracks]."""
    ...
    def empty(self):
        """Empties the Bar, removes all the [refMingusContainersNotecontainer \
NoteContainers]"""
...
NoteContainer]. Raises a !MeterFormatError if the duration is not valid. \
Returns True if succesful, False otherwise (ie. the Bar hasn't got \
...
`place_notes(None, duration)`"""
    ...
    def remove_last_entry(self):
        """Removes the last [refMingusContainersNotecontainer NoteContainer] in the \
Bar"""
...
transpose() on all [refMingusContainersNotecontainer NoteContainers] in \
the bar."""
    ...
    def __setitem__(self, index, value):
        """Allows you to use [] = notation on Bars. The value should be a \
[refMingusContainersNotecontainer NoteContainer], or a \
string/list/[refMingusContainersNote Note] understood by the \
