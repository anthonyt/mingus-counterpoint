...
amplitude, vibrato or other dynamics. Then we want to store the notes in \
bars, the bars in tracks, the tracks in compositions, etc.

We could do this with a number of lists, but ultimately it is a lot easier \
to use objects. The Note class provides an easy way to deal with notes in an \
object oriented matter.

You can use the class [refMingusContainersNotecontainer NoteContainer] to \
group Notes together in intervals and chords."""
...
-2
    ...
    def to_hertz(self, standard_pitch=440):
        """Returns the Note in Hz. The `standard_pitch` argument can be used to set \
the pitch of A-4, from which the rest is calculated."""
