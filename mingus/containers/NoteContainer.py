...
notes and is required for working with [refMingusContainersBar Bars]."""
...
['A-4', 'C-5', 'E-5']
    ...
    def from_interval_shorthand(
        self,
        startnote,
        shorthand,
        up=True,
        ):
        """Empties the container and adds the note described in the startnote and \
shorthand. See core.intervals for the recognized format.
{{{
>>> nc = NoteContainer()
...
['A-4', 'C-5', 'E-5']
    ...
    def is_consonant(self, include_fourths=True):
        """Tests whether the notes are consonants. See the core.intervals module \
for a longer description on consonance."""

        return self._consonance_test(intervals.is_consonant, include_fourths)

    def is_perfect_consonant(self, include_fourths=True):
        """Tests whether the notes are perfect consonants. See the core.intervals \
module for a longer description on consonance."""

        return self._consonance_test(intervals.is_perfect_consonant,
                                     include_fourths)

    def is_imperfect_consonant(self):
        """Tests whether the notes are imperfect consonants. See the core.intervals \
module for a longer description on consonance."""

        return self._consonance_test(intervals.is_imperfect_consonant)

    def is_dissonant(self, include_fourths=False):
        """Tests whether the notes are dissonants. See the core.intervals module \
...
note's name. If no specific octave is given, the note gets removed in \
every octave."""
    ...
    def remove_notes(self, notes):
        """Removes notes from the containers. This function accepts a list of \
[refMingusContainersNote Note] objects or notes as strings and also \
single strings or [refMingusContainersNote Note] objects."""
    ...
    def remove_duplicate_notes(self):
        """A helper function that removes duplicate and enharmonic notes from the \
container"""
    ...
    def get_note_names(self):
        """Returns a list with all the note names in the current container. Every \
name will only be mentioned once."""
...
"""
    ...
    def __setitem__(self, item, value):
        """This allows you to use [] notation on !NoteContainers. This function \
accepts Notes and notes as string.
    Examples:
{{{
>>> n = NoteContainer([\"C\", \"E\", \"G\"])
>>> n[0] = \"B\"
